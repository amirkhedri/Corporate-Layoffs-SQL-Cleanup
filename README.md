# 🧹 Global Tech Layoffs - SQL Data Cleaning Project
...
* **Core SQL Techniques:** Common Table Expressions (CTEs), Window Functions, Self-Joins, String Manipulation, Data Type Casting.
...
* **Engineering Workaround:** To bypass MySQL's strict limitations regarding updating/deleting directly from a CTE, a secondary staging table (`layoffs_staging2`) was instantiated. The partitioned data was migrated, and duplicates were safely dropped using `DELETE`.
...
* **White Space Removal:** Applied the `TRIM()` function to the `company` column to strip invisible, trailing, or leading spaces that would otherwise break `GROUP BY` statements.
...
* **Time-Series Formatting:** The `date` column was imported as raw text. Utilized the `STR_TO_DATE()` function to parse the strings (`'%m/%d/%Y'`), followed by an `ALTER TABLE` command to permanently cast the column to a standard MySQL `DATE` data type.
...
* **Strategic Deletions:** Safely dropped records where both quantitative metrics (`total_laid_off` and `percentage_laid_off`) were `NULL`. Rows missing both critical data points hold zero analytical value for a layoffs analysis.
